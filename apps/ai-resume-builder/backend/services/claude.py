...
class ClaudeService:
    ...
    def _generate_local(self, prompt: str, user_input: str, system_prompt: Optional[str] = None) -> str:
        ...
        sections = [
            "Local demo output",
            "",
            f"System intent: {(system_prompt or 'Helpful assistant').strip()}",
            f"User request: {summary}",
            "",
            "Suggested draft:",
            "1. Goal: Clarify what the user is trying to produce.",
            "2. Key points: Extract the most important requirements from the input.",
            "3. First draft: Create a structured response with headings and bullets.",
            "4. Next step: Ask the user what to refine next.",
            "",
            "Prompt used:",
            rendered_prompt,
        ]
